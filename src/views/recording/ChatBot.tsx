...
const ChatBot = () => {
  ...
  const { startWord, endWord, aiPersonalityName, chatbotSelectedModel } =
    useRecordingStore()
      ...
      if (startIdx !== -1 && endIdx !== -1 && endIdx > startIdx) {
        ...
        if (endIdx > lastTriggerIdxRef.current) {
          ...
          if (questionText) {
            ...
            console.log(
              '[DEBUG] Extracted question text was empty after cleaning',
            )
          }
        } else {
        ...
        if (startIdx > endIdx)
          console.log('[DEBUG] - Start word appears after end word')
      }
    },
    [startWord, endWord],
  )
  ...
  useEffect(() => {
    const transcript = getLastFewMinTranscript
  ...
  useEffect(() => {
    ...
    const generateResponse = async () => {
      try {
        const response = await generateChatBotService(
  ...
  return (
    <div className="bg-white rounded-3xl p-4 no-drag h-full flex flex-col">
      ...
      <div
        className="flex-1 overflow-y-auto space-y-4 mb-2 relative"
      ...
      >
        {chat.length === 0 ? (
            ...
            <div className="w-full bg-gray-100 rounded-lg p-4">
              <div className="text-gray-700 mb-3">
                ...
                <ol className="list-decimal list-inside space-y-1 text-sm">
                  <li>
                    Start with{' '}
                    <span className="font-mono">&quot;{startWord}&quot;</span>
                  </li>
                  <li>Ask anything you want!</li>
                  <li>
                    End with{' '}
                    <span className="font-mono">&quot;{endWord}&quot;</span>
                  </li>
                </ol>
              </div>
              ...
              <div className="bg-white rounded-md border border-gray-200 p-3 text-sm font-mono text-gray-500 overflow-auto max-h-24 whitespace-pre-wrap">
                ...
                <p>
                  <span className="font-semibold">{startWord}</span>. Ask a
                  question <span className="font-semibold">{endWord}</span>.
                </p>
              </div>
            </div>
          </div>
          ...
          chat.map((msg, idx) => (
            ...
            >
              <div
                className={`flex items-end gap-2 ${msg.role === 'ai' ? 'flex-row' : 'flex-row-reverse'}`}
              >
                {/* AI avatar */}
                {msg.role === 'ai' && (
