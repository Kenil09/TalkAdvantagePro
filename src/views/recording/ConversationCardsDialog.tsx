...
const ConversationCardsDialog = () => {
  ...
  useEffect(() => {
    if (open) {
      setFormState({
        selectedModel: conversationCardsSelectedModel || '',
      })
    }
  }, [open, conversationCardsSelectedModel])

  // On save, update Zustand with local form values, which triggers persist/localStorage update
  const handleSave = () => {
    setSelectedModel('conversationCardsSelectedModel', formState.selectedModel)
    setOpen(false)
  }
  return (
...
export default ConversationCardsDialog
