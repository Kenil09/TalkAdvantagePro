...
const ConversationCards = () => {
  const { conversationCards, fetchConversationCards, isLoading, updateConversationCards } =
    useRecordingStore()
  ...
  useEffect(() => {
    if (!liveText) return
    const nextCard = getNextActiveCard(conversationCards, liveText, currentActiveCard)
    if (nextCard) {
  ...
  }, [liveText, conversationCards, getNextActiveCard, currentActiveCard, updateConversationCards])
  ...
  const getCardStyles = (card: Card) => {
    ...
    return {
      className: `${baseClasses} ${extraClass} ${widthClass} ${heightClass} ${isHidden ? 'hidden' : ''} ${animationClass}`,
      zIndex,
      isHidden,
    };
  }
  ...
  return (
    <div className="bg-white rounded-3xl p-4 no-drag h-full flex flex-col">
      ...
      <div className="flex-1 overflow-auto w-full p-2" >
        <div className={getGridLayout()} style={{
          alignItems:
            currentActiveCard?.state === 'elongated' && ["2", "3"].includes(currentActiveCard.id) ? 'end' : 'unset',
        }}>
          {isLoading ? (
          ...
          ) : Array.isArray(conversationCards) && conversationCards.length > 0 ? (
            conversationCards
              .map((card: ConversationCard, index: number) => {
                const activeIndex = conversationCards.findIndex(c => c.id === currentActiveCard?.id)
